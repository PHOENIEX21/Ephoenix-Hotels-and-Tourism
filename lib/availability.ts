...
export const getStayWindow = (checkInDate: string) => {
...
export async function findAvailability(checkInDate: string, guests: number) {
  if (!Number.isInteger(guests) || guests < 1 || guests > 20) throw new Error('Guests must be a whole number between 1 and 20');
  const { checkIn, checkOut } = getStayWindow(checkInDate);

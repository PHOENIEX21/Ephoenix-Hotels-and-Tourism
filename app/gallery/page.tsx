...
export default function GalleryPage() {
  return <main><div className="page-intro"><div className="eyebrow">EPhoenix in focus</div><h1>See the feeling<br />before you arrive.</h1><p>Explore each branch by category. A curated cover image leads every collection in a fixed order. Use the arrows within a category to preview its photos.</p></div><div className="gallery-sections">{['Main', 'Annex I', 'Annex II'].map(branch => <section className="section gallery-branch" key={branch}><div className="section-head"><div className="eyebrow">{branch}</div><h2>{branch} collection</h2></div><div className="gallery-category-grid">{categories.filter(category => category[0] === branch).map(([, category, folder, count]) => <GalleryCategory key={folder} branch={branch} category={category} images={folder === 'annex-ii/special' ? folderImages(`ephoenix/${folder}`, specialImages) : folder === 'annex-ii/swimming-pool' ? folderImages(`ephoenix/${folder}`, swimmingPoolImages) : curatedImageSet(`ephoenix/${folder}`, count)} />)}</div></section>)}</div></main>;
}

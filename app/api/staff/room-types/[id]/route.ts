...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const roomType = await prisma.roomType.update({ where: { id: params.id }, data: { priceKobo: price, depositKobo: deposit } });
    await writeAudit(user.id, 'ROOM_TYPE_PRICING_CHANGED', 'RoomType', params.id, { priceKobo: price, depositKobo: deposit });

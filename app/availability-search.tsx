...
export default function AvailabilitySearch() {
  ...
  return <section id="availability-title" className="availability-panel" aria-labelledby="availability-title">
    ...
    {result && <div className="availability-results"><p className="result-window">{result.nights} night{result.nights === 1 ? '' : 's'} · {new Date(result.checkIn).toLocaleString()} to {new Date(result.checkOut).toLocaleString()}</p>{result.branches.map(branch => <section className="availability-branch" key={branch.slug}><Link className="availability-branch-link" href={`/rooms?hotel=${encodeURIComponent(branch.slug)}`}><h3>{branch.name}</h3><span>View all rooms at this branch →</span></Link>{branch.roomTypes.length ? <div className="availability-list">{branch.roomTypes.map(room => <article className="availability-card" key={room.id}><div className="availability-card-header"><div><div className="availability-card-title">{room.name}</div><p className="availability-card-meta">{room.availableRooms} of {room.totalRooms} rooms available</p></div><div className="availability-card-price">{naira(room.priceKobo * Number(nights))} <small>/ {nights} night{nights === '1' ? '' : 's'}</small></div></div><div className="availability-card-actions"><button type="button" className="availability-action availability-action-secondary" onClick={() => addToCart(room)}>Add to cart</button><Link href={`/book?roomTypeId=${encodeURIComponent(room.id)}&checkIn=${encodeURIComponent(checkIn)}&guests=${encodeURIComponent(guests)}&nights=${encodeURIComponent(nights)}`} className="availability-action availability-action-primary">Book now</Link></div></article>)}</div> : <p className="muted">No rooms match this guest count or date.</p>}</section>)}</div>}
  </section>;
}
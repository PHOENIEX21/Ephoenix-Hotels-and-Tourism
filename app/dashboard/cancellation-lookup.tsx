...
export default function CancellationLookup({ reference = '', email = '', disabled = false }: { reference?: string; email?: string; disabled?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try { const response = await fetch('/api/bookings/cancel', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reference: bookingReference, email: guestEmail }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error); setMessage(`Cancellation ${payload.status}. Refund: NGN ${(payload.refundAmountKobo / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}.`); } catch (cancelError) { setError(cancelError instanceof Error ? cancelError.message : 'Unable to cancel booking.'); } finally { setLoading(false); }
  }

...
export default function RoomDirectory({ hotels, rooms }: { hotels: Hotel[]; rooms: RoomType[] }) {
  ...
  return <div className="room-directory">
    ...
    <div className="room-feed">{visibleRooms.map(room => <article className="card room-card-link" onClick={() => setSelectedRoom(room)} key={room.hotel + room.name}>{room.images?.length ? <RoomCardImage images={room.images} alt={room.name} /> : <div className="room-photo" />}<div className="room-body"><div className="label">{hotels.find(hotel => hotel.slug === room.hotel)?.shortName}</div><h3>{room.name}</h3><div className="price">{naira(room.price)} <small>/ night</small></div><p className="muted">{room.roomNumbers.length} room{room.roomNumbers.length === 1 ? '' : 's'} available in this type · Deposit {naira(room.deposit)}</p><button type="button" className="button button-outline room-preview-button">View room</button></div></article>)}</div>
    {selectedRoom ? <div className="room-modal-backdrop" role="presentation" onClick={() => setSelectedRoom(null)}><section className="room-modal" role="dialog" aria-modal="true" aria-label={`${selectedRoom.name} preview`} onClick={event => event.stopPropagation()}><button type="button" className="room-modal-close" onClick={() => setSelectedRoom(null)} aria-label="Close room preview">×</button>{selectedRoom.images?.length ? <RoomCardImage images={selectedRoom.images} alt={selectedRoom.name} /> : <div className="room-photo" />}<div className="room-body"><div className="label">{hotels.find(hotel => hotel.slug === selectedRoom.hotel)?.shortName}</div><h2>{selectedRoom.name}</h2><p className="muted">Swipe or use the arrows to browse this room&apos;s photos. Images advance automatically.</p><Link className="button button-gold" href={`/book?roomTypeSlug=${encodeURIComponent(selectedRoom.slug)}&hotel=${encodeURIComponent(selectedRoom.hotel)}`}>Book Now</Link></div></section></div> : null}
  </div>;
}
